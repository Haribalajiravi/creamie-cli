...
const componentGenerator = (name, option, path, importPath) => {
    new ComponentGenerator(name, option, [Js, Html, Css, Config], path).start();
    /**
    ...
    fs.readFile(finalImportPath, function (err, data) {
        if (err) throw err;
        if (!data.includes(Utils.capitalize(name)) && !data.includes(componentPath)) {
            fs.appendFile(finalImportPath, `\r\nimport ${Utils.capitalize(name)} from '${componentPath}';`, function (err) {
                if (err) throw err;
