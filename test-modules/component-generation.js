...
describe('Component generation', () => {
    ...
    it('Component creation with no exceptions.', () => {
        try {
            ComponentGenerator(Utilities.componentName, undefined, Utilities.path(), Utilities.importPath());
        } catch (error) {
            throw error;
        }
    });
    ...
    it('Component creation with `-r` option.', () => {
        try {
            ComponentGenerator(Utilities.componentName, '-r', Utilities.path(), Utilities.importPath());
        } catch (error) {
            throw error;
        }
    });
    ...
    after(() => {
        ...
        rimraf(`./${Utilities.baseTestFolder}`, function () {
            console.log(`\tAll the testing junks removed! \n\tTesting done. \n`);
        });
    });
});